...
export function renderVerification() {
  const bodyHtml = `
          ...
          <ul>
            ...
            <li>Animated GIF export, audio export, and a Time Sculpture turntable export were all designed and partially built, then cut rather than shipped without a way to verify them — see <a href="../features/#export">Features</a>. Exported video is currently silent.</li>
          ...
          <p>
            837 unit tests across 83 files, plus 115 Playwright end-to-end tests (87
            desktop, 22 mobile-touch, 6 against a real production build), back all of
            the above; <code>typecheck</code> and <code>build</code> both run clean.
            One flake is currently known: a mobile-only touch test that scrubs, edits,
            compares and opens the Time Sculpture in one continuous journey
            intermittently hits a real, reproducible race in the history-scrub path
            under full-suite load, rather than a click-timing flake.
          </p>
`;

...
export function Timeline() {
  ...
  const clampGen = useCallback((gen: number) => {
    const maxGen = Math.max(currentGenRef.current, historyRing.snapshot().maxGen);
    const windowStart = Math.max(0, maxGen - HISTORY_WINDOW);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const maxGen = Math.max(gen, snap.maxGen);
      const windowStart = Math.max(0, maxGen - HISTORY_WINDOW);
      const shownGen = dragGen ?? gen;

      const key = [snap.version, gen, dragGen, scrubbing, lastCssW, lastCssH, branches.length].join(':');
      if (key !== lastKey) {
        lastKey = key;
  ...
  const onKeyDown = useCallback((e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    switch (e.key) {
      ...
      case 'End': next = Math.max(currentGenRef.current, historyRing.snapshot().maxGen); break;
      default: return;
    }
    e.preventDefault();
